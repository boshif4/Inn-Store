...
//  props استخدمت
// cart,قائمه تحتوي عناصر المنتجات 
//addItem,داله لاضافه المنتجات
 //removeItem,لازاله المنتج 
 //toggleCart، اظهار او اخفاء سله المشتريات
function SideCart({ cart, addItem, removeItem, toggleCart }) {
//subtotal: سعر المنتجات الإجمالي (محسوب ومُحدّث).
//taxes: قيمة الضريبة (محسوبة ومُحدّثة).
//shipping: قيمة الشحن (محسوبة ومُحدّثة).
//total:جمالي المبلغ المطلوب
  const [subtotal, setSubtotal] = useState(0.00);
  const [taxes, setTaxes] = useState(0.00);
  const [shipping, setShipping] = useState(0.00);
  const [total, setTotal] = useState(0.00);

//يقوم بحساب قيم subtotal, taxes, shipping, و total بناءً على عناصر السلة.
  useEffect(() => {
    let subtotal = 0;
    let taxes = 0;
    let shipping = 0;
    ...
    localStorage.setItem("Innovation-cart", JSON.stringify(cart));

  }, [cart]);

//aside: يمثل العنصر الرئيسي لسلة المشتريات.
//cart-header: يمثل رأس السلة (عنوان وإيقاف).
  return (
      <aside className='side side-cart'>
        ...
        <div className="cart-body">
          {cart.length === 0 ? <h5 className='ms-3'>No items selected</h5> : <ul className="list-group">
              {cart.map(item => {
                return (
  ...
  );//يعرض تفصيلاً بالأسعار الفرعية والضرائب والشحن والإجمالي.
}
